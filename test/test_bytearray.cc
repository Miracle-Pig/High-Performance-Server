...
void func(int len, int base_len)
{
    ...
    HPS::ByteArray::ptr ba(new HPS::ByteArray(base_len));
    for (auto &i : vec)
    {
        //! 序列化
        ba->writeInt32(i);
    ...
    {
        //! 反序列化
        uint32_t v = ba->readInt32();
        ASSERT(v == vec[i]);
    }
    ASSERT(ba->getReadSize() == 0);
    LOG_INFO(g_logger) << "func writeInt32/readInt32 (int32_t)len="
                       << len
                       << " base_len=" << base_len
                       << " size=" << ba->getSize();
}
...
#define XX(type, len, write_fun, read_fun, base_len)                            \
    {                                                                           \
        ...
        HPS::ByteArray::ptr ba2(new HPS::ByteArray(base_len * 2));              \
