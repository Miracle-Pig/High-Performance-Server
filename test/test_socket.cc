...
static HPS::Logger::ptr g_looger = LOG_ROOT();

void test_socket()
{
    // std::vector<HPS::Address::ptr> addrs;
    // HPS::Address::Lookup(addrs, "www.baidu.com", AF_INET);
    // HPS::IPAddress::ptr addr;
    // for(auto& i : addrs) {
    //     LOG_INFO(g_looger) << i->toString();
    //     addr = std::dynamic_pointer_cast<HPS::IPAddress>(i);
    //     if(addr) {
    //         break;
    //     }
    // }
    HPS::IPAddress::ptr addr = HPS::Address::LookupAnyIPAddress("www.baidu.com");
    if (addr)
    {
...
void test2()
{
    HPS::IPAddress::ptr addr = HPS::Address::LookupAnyIPAddress("www.baidu.com:80");
    if (addr)
    {
        LOG_INFO(g_looger) << "get address: " << addr->toString();
    }
