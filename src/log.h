...
//# 11) 使用流式方式将日志级别level的日志写入到logger
#define LOG_LEVEL(logger, level)                                                                                 \
    if (logger->getLevel() <= level)                                                                             \
    HPS::LogEventWrap(HPS::LogEvent::ptr(new HPS::LogEvent(logger, level,                                        \
                                                           __FILE__, __LINE__, 0, HPS::GetThreadId(),            \
                                                           HPS::GetFiberId(), time(0), HPS::Thread::GetName()))) \
        .getSS()
...
#define LOG_WARN(logger) LOG_LEVEL(logger, HPS::LogLevel::WARN)

/* 主日志器 */
#define LOG_NAME(name) HPS::LoggerMgr::GetInstance()->getLogger(name)
