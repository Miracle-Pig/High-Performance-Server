...
namespace HPS
{
    ...
    //# 1) 使用指定长度的内存块构造ByteArray
    ByteArray::ByteArray(size_t base_size)
        : m_baseSize(base_size), m_position(0), m_capacity(base_size), m_size(0), m_endian(BIG_ENDIAN), m_root(new Node(base_size)), m_cur(m_root)
    {
    }
    ...
    //# 2) 写入有符号Varint32类型的数据
    void ByteArray::writeInt32(int32_t value)
    {
        //! EncodeZigzag32将value变为正奇数
        writeUint32(EncodeZigzag32(value));
    }
    //# 3) 写入无符号Varint32类型的数据
    void ByteArray::writeUint32(uint32_t value)
    {
        //! 用四个uint8_t代表32位整数???(小端：低位地址存放高位字节)
        uint8_t tmp[5];
        uint8_t i = 0;
        while (value >= 0x80)
        {
