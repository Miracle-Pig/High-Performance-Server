...
namespace HPS
{
    ...
    //# 4.3 使用类型转换模板类偏特化，将Yaml string转换为存储相应类型的容器，如vector：
    /**
    ...
    public:
        ...
        std::string toString() override
        {
            try
            {
                ...
                //! 将m_val类型的Yaml Node转换为Yaml格式字符串
                return ToStr()(m_val);
            }
        ...
        bool fromString(const std::string &val) override
        {
            try
            {
                //! 使用类型转换模板类偏特化,将Yaml格式字符串m_val类型的Yaml Node
        ...
        //! 4.4 转换之后，设定模板参数子类对象的值类型为其转换后的容器类型
        /**
         * @brief 设置当前参数的值
