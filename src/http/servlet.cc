...
namespace HPS
{
    namespace http
    {
        ...
        int32_t FunctionServlet::handle(HPS::http::HttpRequest::ptr request, HPS::http::HttpResponse::ptr response, HPS::http::HttpSession::ptr session)
        {
        ...
        {
            m_default.reset(new NotFoundServlet("HPS/1.0"));
        }
        //# 2) 处理请求，生成响应（递归）
        int32_t ServletDispatch::handle(HPS::http::HttpRequest::ptr request, HPS::http::HttpResponse::ptr response, HPS::http::HttpSession::ptr session)
        {
            //! 获取与路径(uri)相匹配的Servlet
        ...
        void ServletDispatch::addServlet(const std::string &uri, FunctionServlet::callback cb)
        {
            ...
            //! 这里构造FunctionServlet对象自动触发回调函数cb
        ...
        int32_t NotFoundServlet::handle(HPS::http::HttpRequest::ptr request, HPS::http::HttpResponse::ptr response, HPS::http::HttpSession::ptr session)
        {
            response->setStatus(HPS::http::HttpStatus::NOT_FOUND);
            response->setHeader("Server", "HPS/1.0.0");
